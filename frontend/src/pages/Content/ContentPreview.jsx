...
const ContentPreview = () => {
  ...
  const renderPreviewType = () => {
    switch (content.contentType) {
      case 'Video':
        const videoSrc = (!content.fileUrl || content.fileUrl === '#' || content.fileUrl.startsWith('/files/'))
          ...
          <Card sx={{ height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {content.bannerImage && (
                  <Box>
                    <Typography variant="caption" color="text.secondary">Banner Image Preview</Typography>
                    <Box
                      component="img"
                      src={content.bannerImage}
                      alt="Banner Preview"
                      sx={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: 1.5, mt: 0.5, border: '1px solid', borderColor: 'divider' }}
                    />
                  </Box>
                )}
                <Box>
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary">Slug URL</Typography>
                  <Typography variant="body2" fontWeight={600}>{content.slug || 'N/A'}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary">Level & Duration</Typography>
                  <Typography variant="body2" fontWeight={600}>{content.level || 'N/A'} • {content.estimatedDuration || 'N/A'}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary">Language</Typography>
                  <Typography variant="body2" fontWeight={600}>{content.language || 'English'}</Typography>
                </Box>
                <Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  <Box>
                    <Typography variant="caption" color="text.secondary">Brand Color</Typography>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.5 }}>
                      <Box sx={{ width: 20, height: 20, borderRadius: '50%', backgroundColor: content.brandColor || '#6C1D5F', border: '1px solid #ccc' }} />
                      <Typography variant="caption" fontWeight={600}>{content.brandColor || '#6C1D5F'}</Typography>
                    </Box>
                  </Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary">Icon Tag</Typography>
                    <Typography variant="body2" fontWeight={600}>{content.icon || 'None'}</Typography>
                  </Box>
                </Box>
                <Box>
