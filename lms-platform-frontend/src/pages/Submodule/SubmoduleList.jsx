...
const SubmoduleList = () => {
  ...
  const { useList, useDelete } = useSubmodules();
  const { data: submodules = [], isLoading } = useList();
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title="Edit Submodule">
            <IconButton
              size="small"
              onClick={() => navigate(`/submodule/edit/${params.row.id}`)}
  ...
  // Filtering
  const filteredRows = submodules.filter((row) => {
    return (
    ...
    <Box>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <SubtitlesIcon color="primary" sx={{ fontSize: 32 }} />
          <Box>
            <Typography variant="h4" fontWeight={700}>
              Submodules
            </Typography>
            <Typography variant="body2" color="text.secondary">
              List of course submodules. Manage curriculum components within module containers.
            </Typography>
          </Box>
        </Box>
        <Button
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this submodule? You must delete all attached content items first."
